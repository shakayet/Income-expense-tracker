...
> => {
  ...
  for (const scraper of scrapers) {
    ...
    const result = await runScraperSafely(scraper.fn, scraper.name, SCRAPER_TIMEOUT);
    results.push(result);
  }
  ...
  console.log(`🎉 Found ${totalResults} items across ${grouped.length} sources`);
  return {
    found: true,
    data: grouped,
  };
